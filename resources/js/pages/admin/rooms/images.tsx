...
export default function AdminRoomImages() {
    ...
    function cancelDelete(id: number) {
        ...
        const originalImage = room.images.find((img) => img.id === id);
        if (originalImage) {
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        router.post(`/admin/rooms/${room.id}/images`, formData, {
            onSuccess: () => {
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`${room.name} Images - Admin`} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight">
                            Manage Images - {room.name}
                        </h1>
                        <p className="text-muted-foreground text-sm">
                            {room.type} • {room.hotel_name}
                        </p>
                    </div>
                </div>
