...
export default function HotelShow({ hotel, rooms, filters }: Props) {
    ...
    // Filter valid dates and parse them to get the number of nights
        ...
        <>
            <Head title={`${hotel.name} — HotelBD`} >
                ...
                <style>{`
                    *{box-sizing:border-box;margin:0;padding:0}
                    ...
                    /* Gallery Grid */
                    .gallery{display:grid;grid-template-columns:2fr 1fr;grid-template-rows:220px 220px;gap:8px;border-radius:16px;overflow:hidden;margin-bottom:32px}
                    ...
                    .gallery-sub{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:8px}
                    @media(max-width:768px){
                        .gallery{grid-template-columns:1fr;grid-template-rows:300px min-content}
                        .gallery-sub{display:flex;overflow-x:auto;padding-bottom:10px}
                        .gallery-sub .gallery-img-wrapper{flex:0 0 140px;height:100px}
                    }
                    ...
                    .room-img{width:220px;height:160px;border-radius:8px;object-fit:cover;flex-shrink:0}
            ...
            <div className="container">
                ...
                <div className="gallery">
                    <div className="gallery-main gallery-img-wrapper" onClick={() => hotel.images[0] && setSelectedImage(hotel.images[0].path)}>
                        {hotel.images[0] ? (
                            <img src={hotel.images[0].path} className="gallery-img" alt="Main" />
                        ) : (
                    ...
                    <div className="gallery-sub">
                        {[1, 2, 3, 4].map(idx => (
                            <div key={idx} className="gallery-img-wrapper" onClick={() => hotel.images[idx] && setSelectedImage(hotel.images[idx].path)}>
                                {hotel.images[idx] ? (
                                    <img src={hotel.images[idx].path} className="gallery-img" alt={`View ${idx}`} />
                                ) : (
                                    <div style={{ width: '100%', height: '100%', backgroundColor: '#E2E8F0' }} />
                                )}
                            </div>
                            ...
                            {rooms.map(room => (
                                <div key={room.id} className="room-card">
                                    {room.images && room.images.length > 0 ? (
                                        <img src={room.images[0].path} className="room-img" alt={room.name} />
                                    ) : (
            ...
            {selectedImage && (
                ...
                >
                    <button style={{ position: 'absolute', top: 24, right: 24, background: 'none', border: 'none', color: '#fff', fontSize: 36, cursor: 'pointer', zIndex: 10000 }} onClick={() => setSelectedImage(null)}>✕</button>
                    <img src={selectedImage} alt="Fullscreen view" style={{ maxHeight: '90vh', maxWidth: '90vw', objectFit: 'contain', borderRadius: 8, boxShadow: '0 0 40px rgba(0,0,0,0.5)' }} />
                </div>
            )}
        </>
    );
